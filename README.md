...
本机访问 <http://127.0.0.1:5175>；服务器部署时也可通过 `http://公网IPv4:5175` 访问。查看日志和停止服务：
...
- 提交单 P Bilibili 视频任务；
- 解析后预览视频封面；
- 搜索任务，并按处理中、已完成或失败状态筛选；
- 查看任务状态和处理阶段，以简洁提示展示失败原因，并从失败阶段重试；
- 浏览知识库目录并阅读 Markdown，使用快速滚动条浏览长文并导出 PDF。

客户端不会显示或调用 Profile、删除、重新生成和知识重构等管理能力。首次启动需要填写后端根地址：局域网可填写 `192.168.1.20:8000`，部署后可填写 `https://knowledge.example.com`。地址会保存在应用本地，也可以从右上角连接设置中更换。
...
- Docker 默认将 Web 端口 `5175` 监听到所有 IPv4 地址；公网访问还需要在云服务器安全组中放行 TCP 5175。本机启动方式仍只监听本机地址。
